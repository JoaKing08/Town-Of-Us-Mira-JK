        ...
        if (witch != null && !Helpers.GetAlivePlayers().Any(x => x.IsCrewmate() && !x.HasModifier<AllianceGameModifier>()) && !witch.Player.HasDied())
        {
            if (witch.Player.AmOwner)
            {
                var notif1 = Helpers.CreateAndShowNotification(
                    TouLocale.GetParsed("TouModifierEgotistVictoryMessageSelf").Replace("<modifier>", $"{TownOfUsMiraJKColors.Witch.ToTextColor()}{witch.RoleName}</color>"),
            ...
            {
                var notif1 = Helpers.CreateAndShowNotification(
                    TouLocale.GetParsed("TouModifierEgotistVictoryMessage").Replace("<player>", witch.Player.Data.PlayerName).Replace("<modifier>", $"{TownOfUsMiraJKColors.Witch.ToTextColor()}{witch.RoleName}</color>"),
                    Color.white, new Vector3(0f, 1f, -20f), spr: ToUJKRoleIcons.Witch.LoadAsset());
